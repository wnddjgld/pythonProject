...
RESULTS_DIR = "wordprocess/all_results"  # 모든 결과 파일 저장 폴더
...
for pred_time in PREDICTION_TIMES:
    ...
    # 학습 기록 저장
    training_histories[pred_time] = history.history
...
for pred_time in PREDICTION_TIMES:
    ...
    plt.plot(training_histories[pred_time]['loss'], label='Loss')
    plt.plot(training_histories[pred_time]['mae'], label='MAE')
    plt.xlabel('Epochs')
    plt.ylabel('Value')
    plt.legend()
